page 50123 AnalysisbyDimensionsMatCustom
{
    ...
    local procedure CopyDimValueToBuf1(var TheDimCodeBuf: Record "Dimension Code Buffer")
    begin
        with TheDimCodeBuf do begin
            Init;
            Code := 'UNSPECIFIED';
            Name := 'Unspecified';
            // TheDimCodeBuf.SetFilter(Code, 'UNSPECIFIED');
            //if (TheDimCodeBuf.IsEmpty) then
            Insert;
        end;
    end;
    ...
    begin
        ...
        case DimOption of
          ...
          DimOption::"Cash Flow Account":
            ...
            DimOption::"Dimension 1":
                if DimCodeBuf.Totaling = '' then begin
    ...
    local procedure SetDimFilters1(var TheAnalysisViewEntry: Record "Analysis View Entry"; LineOrColumn: Option Line,Column)
    var
        DimCodeBuf: Record "Dimension Code Buffer";
        DimOption: Option "G/L Account",Period,"Business Unit","Dimension 1","Dimension 2","Dimension 3","Dimension 4","Cash Flow Account","Cash Flow Forecast";
    begin
        ...
        case DimOption of
          ...
          DimOption::"Cash Flow Account":
            ...
            DimOption::"Dimension 1":
                if DimCodeBuf.Totaling = '' then begin
                    if DimCodeBuf.Code <> 'UNSPECIFIED' THEN
                        TheAnalysisViewEntry.SetRange("Dimension 1 Value Code", DimCodeBuf.Code)
                    else
                        TheAnalysisViewEntry.SetRange("Dimension 1 Value Code", '');
                end else
                    TheAnalysisViewEntry.SetFilter("Dimension 1 Value Code", DimCodeBuf.Totaling);
            DimOption::"Dimension 2":
                if DimCodeBuf.Totaling = '' then
    ...
    begin
        if AnalysisByDimParameters."Show Actual/Budgets" = AnalysisByDimParameters."Show Actual/Budgets"::"Actual Amounts" then begin
            ...
            if SetColFilter then
                SetDimFilters1(AnalysisViewEntry, 1);
    ...
    begin
        ...
        message(AnalysisByDimParameters."Date Filter");
        AVBreakdownBuffer."Line Code" := Code;
        if ColumnCode = '' then
            ColumnCode := 'Unspecified';
        AVBreakdownBuffer."Column Code" := ColumnCode;
        AVBreakdownBuffer.Amount := Amount;
        AVBreakdownBuffer.SetFilter("Line Code", code);
        AVBreakdownBuffer.SetFilter("Column Code", '%1', ColumnCode);

        if (AVBreakdownBuffer.IsEmpty) then begin
            //   Message('%1   , %2     , %3', AVBreakdownBuffer."Line Code", AVBreakdownBuffer."Column Code", AVBreakdownBuffer.Amount);
            AVBreakdownBuffer.Insert();
        end;
        /*        if CheckGL = Code then begin
                    if ColumnCode <> CheckDim then
                        DiffAmount := DiffAmount - Amount;

                end else begin
                    Message(CheckGL);

                    AVBreakdownBuffer.SetFilter("Line Code", CheckGL);
                    // AVBreakdownBuffer.SetFilter("Column Code", 'UNSPECIFIED');
                    if AVBreakdownBuffer.Find()
                    then begin
                        Message('%1, %2, %3', AVBreakdownBuffer."Line Code", AVBreakdownBuffer."Column Code", AVBreakdownBuffer.Amount);
                        // AVBreakdownBuffer1.Get(code, ColumnCode);
                        AVBreakdownBuffer.Amount := DiffAmount;
                        AVBreakdownBuffer.Modify();
                    end;
                    CheckGL := Code;
                    //if ColumnCode = 'Unspecified' then
                    CheckDim := ColumnCode;
                    DiffAmount := Amount;
                end;
                IF DiffAmount > 0 THEN
                    Message('Diff Amount --> %1', DiffAmount);*/
        exit(Amount);
    end;
    ...
    begin
        ...
        for i := 1 to 32 do begin
            if (NewMATRIX_ColumnCaptions[i] = '') and (check = 0) then begin
                NewMATRIX_ColumnCaptions[i] := 'Unspecified';
                check := 1;
                index := i;
            end;
        end;

        for iterI := 1 to index - 1 do
            for IterJ := iterI + 1 to index do
                if NewMATRIX_ColumnCaptions[iterI] > NewMATRIX_ColumnCaptions[iterJ] then begin
                    temp := NewMATRIX_ColumnCaptions[iterI];
                    NewMATRIX_ColumnCaptions[iterI] := NewMATRIX_ColumnCaptions[iterJ];
                    NewMATRIX_ColumnCaptions[iterJ] := temp;
                end;
    ...
    begin
        ...
        //end;
        CopyDimValueToBuf1(DimensionCodeBuffer);
